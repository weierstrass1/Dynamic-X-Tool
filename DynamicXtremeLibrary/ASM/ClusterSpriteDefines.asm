!Version = !ClusterVersion
!LastVersion = !ClusterLastVersion

!PoseIndex = !ClusterPoseIndex
!LastPoseIndex = !ClusterLastPoseIndex
!AnimationIndex = !ClusterAnimationIndex
!AnimationPoseIndex = !ClusterAnimationPoseIndex
!AnimationTimer = !ClusterAnimationTimer
!GlobalFlip = !ClusterGlobalFlip
!LocalFlip = !ClusterLocalFlip
!Palette = !ClusterPalette
!PaletteAssignmentOption = !ClusterPaletteAssignment
!PaletteOption = !ClusterPaletteOption
!LastPoseHashIndex = !ClusterLastPoseHashIndex

!RenderXDistanceOutOfScreen = !ClusterRenderXDistanceOutOfScreen
!RenderYDistanceOutOfScreen = !ClusterRenderYDistanceOutOfScreen

!SafeFrameLowByte = !ClusterSafeFrameLowByte
!SafeFrameHighByte = !ClusterSafeFrameHighByte

!LastFlip = !ClusterLastFlip

!State = !ClusterState

!HitboxTableB = !ClusterHitboxTableB
!HitboxTableH = !ClusterHitboxTableH
!HitboxTableL = !ClusterHitboxTableL

!PlayerIsAbove = !HitmanClusterPlayerIsAbove

!RenderBox = %DXClusterRenderBox()
!PaletteAssignment = %DXClusterPaletteAssignment()
!PaletteAssignmentNoUpload = %DXClusterPaletteAssignmentNoUpload()
!Upload = %DXClusterUpload()
!UploadWithoutPoseOffset = %DXClusterUploadWithoutPoseOffset()
!DXDraw = %DXClusterDraw()
!DXDrawWithOffset = %DXClusterDrawWithOffset()
!ConcatenateRGBPaletteEffectToGlobal = %DXClusterConcatenateRGBPaletteEffectToGlobal()
!ReplacePalette = %DXClusterReplacePalette()
